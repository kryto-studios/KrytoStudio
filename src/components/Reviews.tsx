...
export default function Reviews() {
  ...
  const checkPermissionAndExistingReview = async () => {
    ...
    try {
      ...
      if (permissionData) {
        ...
        const existing = reviews.find(r => r.email.toLowerCase().trim() === user.email?.toLowerCase().trim());
        if (existing) {
  ...
  // Hide the section completely if there are no reviews AND the current user is NOT authorized to write one.
  if (!loading && reviews.length === 0 && !authorized) {
    return null;
  }
    ...
    <section id="reviews" className="relative z-20 bg-background pb-32 pt-10 px-6 border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          {/* Magical Authorized Button: ONLY shown if user has review permission */}
          {authorized && (
            <motion.div 
            ...
            >
              {!userReview ? (
                <button
                  ...
                  className="bg-accent hover:bg-accent/90 text-white font-bold py-3 px-8 rounded-full text-sm transition-all shadow-[0_0_20px_rgba(14,165,233,0.3)] flex items-center gap-2 cursor-pointer"
                >
                  <MessageSquare size={16} />
                  {showForm ? "Close Review Composer" : "Write a Verified Review"}
                </button>
              ) : (
                !isEditing && !showForm && (
                  <button
        ...
        {/* Dynamic Form Area inside landing page Reviews section */}
        <AnimatePresence>
          {authorized && showForm && (
            <motion.div 
                ...
                <div className="flex gap-3">
                  ...
                  {userReview && (
                    <button 
                      ...
                      className="bg-red-500/10 hover:bg-red-500/20 border border-red-500/10 text-red-400 font-semibold py-3 px-5 rounded-xl transition-all text-xs flex items-center gap-1.5 cursor-pointer"
                    >
                      Delete Review
                    </button>
                  )}
